<script lang="ts">
    ...
    import PresentCell from "./PresentCell.svelte";
    ...
    async function create_slot(event: Event) {
        ...
        try {
            ...
            if (result.valid) {
                ...
                console.log("Starts at :", formData.date.starts_at);
                console.log("Ends at   :", formData.date.ends_at);
                ...
                $flash = { // Flash message en cas d'erreur de validation
                    type: "error",
    ...
    const columns = table.createColumns([
        ...
        // Attendee boolean -> request prisma for that
        table.column({
            accessor: ({ id }) => id,
            header: "Présent",
            cell: ({ value }) => {
                return createRender(PresentCell, { userId: value })
            }
        }),
        /*table.column({
            accessor: "first_name",
...
<Card class="flex flex-col justify-between w-full">
    {#if current_slot.id != "0"}
        {#if (user.root || data.user.id === current_slot.owner_id)}
